...
begin
  ...
  if v_n = 0 then
    raise exception
      'LNI 034_multitenancy_foundation: no auth.users row matches <PLATFORM_EMAIL>.';
  end if;

  if v_n > 1 then
    raise exception
      'LNI 034_multitenancy_foundation: multiple auth.users rows match <PLATFORM_EMAIL>.';
  end if;
  ...
  if v_platform is null then
    raise exception
      'LNI 034_multitenancy_foundation: auth.users row matching <PLATFORM_EMAIL> is not email-confirmed.';
  end if;
  ...
  select e.owner_id, 'display_name', '<OWNER_NAME>'
