...
- `LNI_TENANT2_TELEGRAM_USER_ID`: `<LNI_TENANT2_TELEGRAM_USER_ID>` — second-account identifier. Real value belongs only in gitignored `docs/environment.local.md`. Used at apply time for migration `041` (`SET lni.tenant2_telegram_user_id`); never `$env`. Never a literal in a committed file.
