...
| 27 | **A `person_id` used after an upsert comes from that upsert's own result, never from a name lookup.** | G1. Live WF-05 `b6cd3894`: no-email person binds to **every** same-name person (SQL sim: 3 rows). Packet 13.3-R STOP-1 is the resolved-id PUT. #46/#208 **NOT repaired**. |
| 28 | **The watchdog never requeues an enrichment job.** A stuck spend is parked `needs_review`; a provider call is never repeated by recovery. | G2. Packet 13.3-R PUT on WF-09 (named rollback `b3dedb40`): `stuck_running` + `job_type=enrichment` → `park_ids`, never `requeue_ids`. |
| 29 | **A provider balance that cannot be read blocks the spend; an unknown delta is recorded as 1, never NaN.** | Packet 13.3-R PUT on WF-06 (named rollback `c0d7a773`). IF Credits readable?; else Release: credit read failed (no Apollo, no ledger). Write match `$10` = before−after if finite and ≥0, else 1. |
