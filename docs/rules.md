...
| 27 | **A `person_id` used after an upsert comes from that upsert's own result, never from a name lookup.** | G1. Live WF-05 `b6cd3894`: no-email person binds to **every** same-name person (SQL sim: 3 rows). `person_hit` is still `p.full_name = s.full_name`. #46 `cc224878` interaction `5ae91209` bound `c747ab72` not minted `a21a803f`. #208 `06ddeeeb` interaction `5a90989d` bound `4151e101` not minted `3e442ace`. Do **not** repair #46/#208. Packet 13.3 records the live fan-out; resolved-id PUT is later. |
| 28 | **The watchdog never requeues an enrichment job.** A stuck spend is parked `needs_review`; a provider call is never repeated by recovery. | G2. Packet 13.3 PUT on WF-09 (named rollback `b3dedb40`): `stuck_running` + `job_type=enrichment` → `park_ids`, never `requeue_ids`. |
| 29 | **A provider balance that cannot be read blocks the spend; an unknown delta is recorded as 1, never NaN.** | Packet 13.3 PUT on WF-06 (named rollback `c0d7a773`). IF Credits readable?; else Release: credit read failed (no Apollo, no ledger). Write match `$10` = before−after if finite and ≥0, else 1. |
...
`<WF10_ROLLBACK_DESRAJ>`). S5 docs cleaned. S6 cause written, **not fixed**.
**10.2c** S7a/S7b live (WF-02 `ce51e6f4`, WF-04 `6fa41bc4`
after 10.2c-fix). S8 logged, not fixed. **S9** logged
7 Sep, not fixed. Phone prove R1–R5 still required.
...
| **S6** | Person minted with no interaction row. <CONTACT_16_NAME> `32c8efee-c0a9-4ef9-bac8-f645536d93af`, 31 Aug 13:28Z, `source_type=card`, from capture **#153**. `structured_output` contains `blossommena`. Extraction succeeded. WF-05 created the person. **No `interactions` row.** #153 is `ready`. **Cause (14.0 C2 + G1):** pre-13.2 `Insert interaction` `LIMIT 1` plus `NOT EXISTS (capture_id)` — one interaction per capture. **13.2 LIVE** (`b6cd3894` + catalog 045 unique): one row per extracted person, `ON CONFLICT (capture_id, person_id)`. **G1 remains:** no-email name bind hits **every** same-name person (sim 3 rows). #46/#208 **NOT repaired**. Live NULL-person interactions: **20** (17 live owner + 3 test; 5 note replays 17 Sep 10:00Z). Do not write an interaction row by hand. Do not backfill. |
...
| **S9** | Replay-minted person has no `interactions` row. Same guard as S6. #151 <CONTACT_36_NAME> `96066d72`. **13.2 LIVE** (same PUT as S6). Do not UPDATE `person_id` by hand. Do not backfill. |
...
- archived orphans `<ORPHAN_WF_A_ID>` and `<ORPHAN_WF_B_ID>` — verify by
