...
| 27 | **A `person_id` used after an upsert comes from that upsert's own result, never from a name lookup.** | G1. #46 `cc224878` interaction `5ae91209` bound `c747ab72` not minted `a21a803f`. #208 `06ddeeeb` interaction `5a90989d` bound `4151e101` not minted `3e442ace`. Upsert `inserted:1`; person_hit `p.full_name = x.full_name` `LIMIT 1` no `ORDER BY`. Packet 13.1. |
| 28 | **The watchdog never requeues an enrichment job.** A stuck spend is parked `needs_review`; a provider call is never repeated by recovery. | G2. Packet 13.1. Live WF-09 Compose still maps `stuck_running && attempts < 3` → `requeue_ids` (named rollback `b3dedb40`). Not PUT this turn. |
| 29 | **A provider balance that cannot be read blocks the spend; an unknown delta is recorded as 1, never NaN.** | Packet 13.1. Live WF-06 Write match `$10` is `Math.max(0, before−after)` — non-2xx body → `Number(undefined)=NaN`. Named rollback `c0d7a773`. Not PUT this turn. |
...
| **S6** | Person minted with no interaction row. <CONTACT_16_NAME> `32c8efee-c0a9-4ef9-bac8-f645536d93af`, 31 Aug 13:28Z, `source_type=card`, from capture **#153**. `structured_output` contains `blossommena`. Extraction succeeded. WF-05 created the person. **No `interactions` row.** #153 is `ready`. **Cause (14.0 C2 + G1):** `Insert interaction` `LIMIT 1` plus `NOT EXISTS (capture_id)` — one interaction per capture; first join wins. Name join after upsert bound a namesake, not the minted id (#46 `c747ab72` not `a21a803f`; #208 `4151e101` not `3e442ace`). **Design = packet 13.1** (resolved ids from upsert, one row per DISTINCT `person_id`, planned 046 unique). #46/#208 **NOT repaired** (owner decision pending). Packet said 15 NULL-person interactions stay as evidence; **live 20** (17 live owner + 3 test). Do not write an interaction row by hand. Do not backfill. |
