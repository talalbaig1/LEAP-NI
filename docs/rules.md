...
| 25 | **No literal identifier, uuid, email, host, credential id, workflow id or webhook path in any committed file**, including session logs and packet plans. Session logs cite tokens (`<OWNER_ID>`, `<WF01_ID>`, `<CONTACT_1_EMAIL>`). Real values live only in gitignored `docs/environment.local.md`. | 12.5a-0b: eleven categories of leak across 119 commits. The current session-log style produced it. `scripts/check-no-literals.sh` + CI is the control. History rewrite is a later packet. |
