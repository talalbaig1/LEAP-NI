...
- `f210d77d-a6b2-45b7-9e10-3668a51bc4fa` — **cancelled** in 9.11
  (was awaiting_confirm to <CONTACT_19_NAME>). Do not re-send. Do not treat as
  open.

People: `9489be75` <CONTACT_17_NAME>, `ec5dc966` LNI Followup Prove,
`d8b051cb` <CONTACT_18_NAME>, `09dfa793` <CONTACT_19_NAME>,
`4151e101` <CONTACT_20_NAME> (`shared_contact`),
...
WF-01 followup senders are **HTML** (`<WF01_PUBLISHED>`). Compose
HTML-escapes. Do not revert to Markdown. Byte 521 / exec
**278965** was `_` in a filename under Markdown.

WF-02 `<WF02_PUBLISHED>`: followup `/done` runs the same enqueue as
...
webhook `/webhook/<TEST_716_DRIVER_PATH>` is archived.
