...
WF-01 draft `<WF01_DRAFT>` or the WF-06 draft `<WF06_DRAFT>`. If
...
(WF-00 `<WF00_ID>`, never ElderWise), `executionTimeout`
...
each type on the instance — ElderWise Postgres `<ELDERWISE_PG_CRED_ID>`
and a random Telegram bot. The create response is not evidence.

After every create, REST PUT settings **and** rebind:

- postgres **Leap-NI** `<PG_CRED_ID>`
- telegramApi **Leap-NI** `<TELEGRAM_CRED_ID>`
- httpHeaderAuth **Supabase_Leap-NI** `<STORAGE_CRED_ID>`
- openAiApi **OpenAi account** `<OPENAI_CRED_ID>`
- gmailOAuth2 **Gmail** `<GMAIL_CRED_ID>`
- Apollo **Apollo Leap-NI** `<APOLLO_CRED_ID>`
- Tavily **Tavily Leap-NI** `<TAVILY_CRED_ID>` — never generic
  `Tavily account`
...
`LNI-TEST-7.16-driver` `<TEST_716_DRIVER_WF_ID>`, webhook
`POST {N8N_BASE_URL}/webhook/<TEST_716_DRIVER_PATH>`.
...
`POST /webhook/<WF01_INGEST_PATH>` wired into **Allowlist**. n8n
...
2. **GET WF-01** name + **`activeVersionId`**. Must be
   ...
   `<WF01_PUBLISHED>`. Draft
   `versionId` `<WF01_DRAFT>` is the 05:20 canvas autosave —
   not a PUT. If `activeVersionId` moved, STOP and report.
   If the **draft** id moved off `<WF01_DRAFT>`, someone wrote
   again — STOP and report. Same for WF-06 draft `<WF06_DRAFT>`
   / published `<WF06_PUBLISHED>`.
