...
`<PLATFORM_EMAIL>` (dotless). No `bot_state`.
...
|---|---|---|---|---|---|---|
| 00 | `<WF00_ID>` | true | `be1e7b71-b55a-4bd8-a318-fa096ae0257d` | same | **12.2 rollback `<WF00_PUBLISHED>`** | 15 |
...
| 07 | `<WF07_ID>` | true | `<WF07_PUBLISHED>` | same | **12.4b packet restore `<WF07_ROLLBACK>`** (named **before** first 12.4b PUT). Immediate predecessor of `<WF07_PUBLISHED>` is `feb5f066` (named before that PUT). Chain `9197f7a3` (12.2) → `becd329b` (12.2a) → `<WF07_ROLLBACK>` (12.3b) → `<WF07_PUBLISHED>` (12.4b). Intermediates `353f649a` → `feb5f066` recorded 12.4c after the fact. `<WF07_ROLLBACK>` is not 12.2a. | 34 |
| 08 | `<WF08_ID>` | true | `8b835659-6179-4472-9f78-f096bdaba841` | same | **12.2 rollback `<WF08_PUBLISHED>`** | 19 |
| 09 | `<WF09_ID>` | true | `<WF09_PUBLISHED>` | same | named `<WF09_ROLLBACK>` | 43 |
| 10 | `<WF10_ID>` | true | `<WF10_PUBLISHED>` | same | **12.5a-0 rollback `<WF10_ROLLBACK>`** | 171 |
...
- MCP `execute_workflow` cannot run an
  ...
  `POST /webhook/<WF10_HISTORY_PATH>` — **removed 12.5a-0**.
...
`<WF01_PUBLISHED>` still has unauthenticated `Driver ingest`
