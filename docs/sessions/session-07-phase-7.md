...
| 7.3 | live n8n + #38 | WF-10 `<WF10_ID>` created **INACTIVE**. Command confirm, Gmail To+CC, double-send. Attachment path **reported built, was not**. |
...
**WF-10, live.** id `<WF10_ID>`, name `LNI WF-10 - Follow-up
...
LNI-TEST-2.2 capability spike `<TEST_22_SPIKE_WF_ID>` is older and
...
- Signup / Confirm email / delete `<STRAY_AUTH_USER_ID>` still owner.
...
on disposable `LNI-TEST-WF10-buttons` (`<TEST_WF10_BUTTONS_WF_ID>`): fixed
...
- Prove: TEST `LNI-TEST-WF10-voice-exec` `<TEST_WF10_VOICE_WF_ID>`
