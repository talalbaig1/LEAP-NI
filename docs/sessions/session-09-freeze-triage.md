...
|---|---|---|---|
| 01 | `<WF01_DRAFT>` | `<WF01_PUBLISHED>` | 05:20:14Z |
| 06 | `<WF06_DRAFT>` | `<WF06_PUBLISHED>` | 05:21:01Z |
...
publish drafts `<WF01_DRAFT>` (WF-01) / `<WF06_DRAFT>` (WF-06).
...
- WF-01 / 02 / 03 / 04 / 05 ran on the **published**
  versions. Drafts `<WF01_DRAFT>` / `<WF06_DRAFT>` stayed
...
Verified independently: #160 person is <CONTACT_10_NAME>
`<<CONTACT_10_EMAIL>>` `source_type=shared_contact`.
#161 typed_note `"He is from fathom.io"` retained.
Published WF-10 `<WF10_PUBLISHED_9_14>` contains zero `UPDATE captures`.
...
| t (Z) | exec | what |
|---|---|---|
| 13:26:19 | WF-01 **317924** Route type[6] contact → lastNode `Contact reply sent` | `Contact saved · #160 · <CONTACT_10_NAME>` (msg 641) |
...
Published WF-10 `<WF10_PUBLISHED_9_14>` writes no capture status
...
<CONTACT_16_NAME> `32c8efee-c0a9-4ef9-bac8-f645536d93af` is not
missing an interaction because WF-05 skipped the node.
`Insert interaction` ran on #153. It wrote **one** row,
`person_id` = <CONTACT_13_NAME> `4efe1828` (people[0], note, no
...
| **<CONTACT_16_NAME>** | `32c8efee` | **#153 people[1]**; ix → <CONTACT_13_NAME> |
| <CONTACT_13_NAME> | `d887ab79` | #152 name-duplicate of `4efe1828` |
| <CONTACT_44_NAME> | `39d9fffa` | **#170 people[1]**; ix → <CONTACT_44_NAME> |
| <CONTACT_47_NAME> | `ac362f34` | **#171 people[1]**; ix → <CONTACT_48_NAME> |
| <CONTACT_46_NAME> | `412f5aa4` | **#176 people[1]**; ix → <CONTACT_49_NAME> |
...
Rollback before PUT: WF-02 `<WF02_PUBLISHED>`, WF-10 `<WF10_PUBLISHED_9_14>`.
...
|---|---|---|---|
| 02 | `<WF02_PUBLISHED>` | **`201095c6-8aab-4503-8532-aedafb358d90`** | 91 → 95 |
| 10 | `<WF10_PUBLISHED_9_14>` | **`<WF10_ROLLBACK_DESRAJ>`** | 146 → 150 |

Settings unchanged: `Asia/Riyadh`, error WF-00, MCP true,
timeout 300. WF-05 still `<WF05_PUBLISHED>`. WF-01 published still
...
Live WF-05 `<WF05_PUBLISHED>` node `Set capture status`:
...
### WF-04 gate (proved before PUT, <WF04_PUBLISHED>)
...
Telegram contact names are owner labels ("<CONTACT_38_NAME>"). They become `full_name` only when they are
the only name. A cleaner asset-derived name is kept.

#156 owner-confirmed same man: v6 keeps asset/note name
`<CONTACT_11_NAME>`, fills `<CONTACT_11_EMAIL>` and
...
Rollback before PUT: WF-02 `201095c6`, WF-04 `<WF04_PUBLISHED>`.
...
| 04 | `<WF04_PUBLISHED>` | **`dafe9b02-4523-4936-9077-4bf975f998aa`** | 28 → 29 |

Settings unchanged. WF-05 still `<WF05_PUBLISHED>`. T1–T4: owner's
...
ready, no duplicate. contact-v1 `"<OWNER_NAME>"` email
...
4. Cause: `Load labelled sources` had `contact_run`
   (`<OWNER_NAME>`). `Build labelled sources` does not
...
= <CONTACT_36_NAME> + email + phone, `source=shared_contact`.
...
|---|---|---|---|
| 157 | <CONTACT_6_NAME> `58ea7ec0` | <CONTACT_6_EMAIL> | +966 55 667 7268 |
| 156 | <CONTACT_11_NAME> `4ef3824c` | — | 0538584129 |
| 165 | <CONTACT_39_NAME> `94f6d2b2` | — | 0509609942 |
| 184 | <CONTACT_42_NAME> `4e51b68d` | — | +923000334560 |
| 185 | <CONTACT_37_NAME> `8ae2ea22` | — | +447545222169 |
| 203 | <CONTACT_51_NAME> `60ff201e` | — | 966501690331 |

#156 did **not** fill onto <CONTACT_11_NAME>. Zero named
asset people → Parse adopted contact-v1 wholesale
(`<CONTACT_11_NAME>`). Incoming email null; Rana
...
<CONTACT_51_NAME> is linked. Watchdog Telegram fired (stuck
