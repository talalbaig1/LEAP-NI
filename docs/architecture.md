...
| `captures` | One `/new`…`/done` unit | `event_id`, `capture_no`, `status`, `capture_mode`, `opened_at`, `closed_at`, `close_reason`, `typed_note`, `flags` (`media_group_id` lives at `flags->>'media_group_id'`) |
...
packet 1.4; the mechanism and index settle the schema now.

Migration 003 defaulted `flags` to `'[]'::jsonb` (array). The index
predicate `flags ? 'media_group_id'` is false for that default, which is
what we want for non-album captures. Packet 1.4 writes an **object**
`{"media_group_id": "<id>"}` for album captures. Do not store the group
id as an array element.
...
| Auto-expose new tables | **Disabled** | Numbered migrations (`001`–`013`) create the 16 tables, indexes, policies, bucket, seed, and the processing-job staleness column. Auto-expose plus one missed policy equals publicly readable contact data. |
