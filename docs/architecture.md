...
| 041 | `041_tenant2_bot_state` | Packet 12.8. **Not applied** until job `7c72371f` ages out of `failed_24h` (`2026-09-17 11:07:59Z`). Inserts ONE `bot_state` for the test tenant (`mode=normal`, `open_capture_id` NULL). `telegram_user_id` from `current_setting('lni.tenant2_telegram_user_id', true)` — RAISE missing/empty. Owner from `events.name = 'NIS test tenant'`. Never a hardcoded uuid or telegram id. Asserts 034 `bot_state_telegram_user_id_key`. No `digest_email`. 030 stays Phase 6. |
