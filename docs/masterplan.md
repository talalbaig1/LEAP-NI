...
| D-N | Product rename | Product name is **Networking Intelligence System (NIS)**. Internal identifiers do **not** change: `lni_config`, `lni_settings`, `lni_public_suffixes`, `lni_free_email_domains`, `lni_normalize_domain`, bucket `lni-assets`, all `*_owner_all` policies, catalogued migration names, the `LNI ` / `LNI-TEST-` workflow-name prefix, the repo path. That prefix is the safety rule that keeps n8n API calls off ElderWise. `LNI` is the legacy internal code prefix. **User-facing Telegram/Gmail copy is NIS as of packet 14.0** (watchdog, morning briefing, day close, repeated failure, History evidence) | Owner 14 Sep, scoped by architect. 14.0 B1 applied. |
...
| D-P | Card is truth; enrichment is context | What the draft ASSERTS about a person comes from their card. Apollo may be stale or wrong — <CONTACT_33_NAME>'s card reads "Solution Specialist", Apollo reads "Connectivity Consultant, seniority entry". Enrichment informs the composer's brief; it never becomes a sentence claiming their title | Architect 15 Sep (Phase 13). After 12.2. Packet 13.1. |
| D-Q | Enrichment as evidence | Enrichment surfaces as evidence beside the draft in Telegram (D-F), never silently inside a body | Architect 15 Sep (Phase 13). After 12.2. Packet 13.1. |
| D-R | Channel stays the card | Channel pick stays the **card**: email / phone / `people.linkedin_url`. An Apollo LinkedIn URL must not flip a person onto the LinkedIn channel. It may appear in evidence only | Architect 17 Sep. `docs/plans/phase-13-plan.md`. Locked from PR #97 wording. |
| D-S | Curated enrichment columns | Curated columns, never `payload::text`. Hollow rows (`payload = '{}'` or blank `name` on person/apollo) join as NULL. Latest `fetched_at` wins. Every join is `enrichment_records.owner_id = people.owner_id` | Architect 17 Sep. `docs/plans/phase-13-plan.md`. Locked from PR #97 wording. |
| D-T | `/ask` stays enrichment-blind | Phase 6 deliberate. WF-07 digest also does not read `enrichment_records`. WF-06 write path unchanged | Architect 17 Sep. `docs/plans/phase-13-plan.md`. Locked from PR #97 wording. |
| D-U | Echo flag, never silent edit | If a draft repeats an Apollo title/headline that differs from the card, the confirm card flags it. The system never edits the draft silently. **Not built.** Withdrawn packet 13.1 numbered this D-S; that letter is D-S curated columns. This decision keeps the D-U letter | Architect 17 Sep. Withdrawn 13.1 echo-flag, renamed. |
