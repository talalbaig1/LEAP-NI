...
- **#12** — Decision 12 (draft-only, no auto-send) **STANDS unchanged.**
  ...
  `<WF10_ROLLBACK_9_14>`. WF-01 stays
  `<WF01_ROLLBACK>` (Phase 9 PUT
...
| 7 | Follow-up as a capture; deferred complete when the person appears | LIVE. WF-10 `<WF10_ROLLBACK_9_14>`. Immediate `/done` + WF-05 `source=deferred` fallback. |
