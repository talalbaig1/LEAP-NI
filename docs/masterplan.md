...
| D-R | Fenced CONTEXT only | Enrichment reaches the composer as a fenced CONTEXT block the model may use for tone/relevance only; it must not assert title, seniority, employer, history or headline from it | Architect 17 Sep. Packet 13.1. Replaces PR #97 “channel stays the card” D-R. |
| D-S | Echo flag, never silent edit | If a draft repeats an Apollo title/headline that differs from the card, the confirm card flags it. The system never edits the draft silently | Architect 17 Sep. Packet 13.1. Replaces PR #97 “curated columns” D-S. |
| D-T | `/ask` stays enrichment-blind | Phase 6 deliberate. WF-07 digest also does not read `enrichment_records` | Architect 17 Sep. Packet 13.1 Step 7. |
