...
| 2 | Create new Supabase project, paid tier (~2 GB) | Talal | **DONE** — project `LEAP-NI`, `eu-central-1`, Pro, Postgres 17.6. Migrations `001`–`008` applied. `009` seed blocked until the owner Auth user exists. `010` (`last_transition_at`) is additive and does not wait on Auth users. | — |
