...
| D-R | Channel stays the card | Apollo LinkedIn URL must not flip channel. Evidence only | Architect 17 Sep. `docs/plans/phase-13-plan.md`. |
| D-S | Curated enrichment columns | Never stringify `payload`. Hollow rows join NULL. Latest `fetched_at`. Join `owner_id` | Architect 17 Sep. `docs/plans/phase-13-plan.md`. |
| D-T | `/ask` stays enrichment-blind | Phase 6 deliberate. WF-07 digest also does not read `enrichment_records` | Architect 17 Sep. `docs/plans/phase-13-plan.md`. |
