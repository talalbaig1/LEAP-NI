...
| Item | Choice |
|---|---|
| Provider | OpenAI (same credential `<OPENAI_CRED_ID>` already on WF-04/08/10) |
...
WF-08 id `<WF08_ID>` is **ACTIVE**. versionId
`<WF08_PUBLISHED>`. Live `Retrieve corpus` is:
...
5. PUT WF-08 without `active`. Strip `binaryMode` /
   ...
   `<WF08_PUBLISHED>`. Do not deactivate.
...
Live WF-07 (`<WF07_ID>`, versionId
`<WF07_PUBLISHED_9_14>`), node `Load digest`.
