...
- Additive schema only. Forward-only migrations. Catalog **name**
  for embeddings starts `028_`. **027 is taken:** packet 7.8-FIX
  `027_follow_ups_brief` (`follow_ups.brief` + script flags).
  **026 is taken:** GATE-FIX `026_captures_last_activity`.
  Embeddings is **028**, not 027.
...
migration `001_extensions`. Migration 028 must
...
on the image. It is not in `pg_extension` until 028.

---

## B. Schema — migration 028, additive

**File:** `supabase/migrations/028_embeddings.sql`
**Catalog name:** `028_embeddings` (must start `028_`; 027 is
`027_follow_ups_brief`; 026 is `026_captures_last_activity`; 023 is
still the unnamed `processing_jobs_enrichment_person_uniq`; 012 never
applied).

028 does four things and nothing else:
...
crosses tens of thousands — that is a later packet, not 028.
...
- New table. New extension. New indexes. New RLS policy on the new
  table only.
- Dropping 028 is `drop table embeddings; drop extension vector;`
  if nothing else depends on it. Do not plan to drop it.
- WF-08 is unchanged by 028. Retrieve still works on trigram if
  028 lands and the WF-08 packet never does.
...
- `processing_jobs_job_type_check` is a closed list
  ...
  `embed` is an ALTER CHECK on a live table. That is not "028 is
...
028 can land without touching WF-08. Backfill can land without
...
Do not combine 028, WF-11, and the WF-08 PUT in one packet.
...
**Recommended floor for the later WF-08 packet (not 028):** keep a
row if **either**
...
| **6.1 Schema** | migration `028_embeddings` applied | after the event, or when the architect says | `pg_extension` has `vector` 0.8.2. `embeddings` exists, 0 rows, HNSW present. Catalog name starts `028_`. No ALTER of `processing_jobs_job_type_check`. WF-08 GET unchanged. |
...
**During 31 Aug – 3 Sep:** no 028, no WF-11 schedule, no WF-08 PUT.
...
| Trap | How avoided |
|---|---|
| `installed_version` null mistaken for "pgvector missing" | It is available, not installed. 028 CREATE EXTENSION. |
...
| WF-08 PUT in the same packet as 028 | Forbidden. §F. |
...
| 023 unnamed catalog | File **and** catalog `028_…`. 027 is `027_follow_ups_brief`. 026 is last_activity. |
...
2. **Backfill as a WF-03 job type** — refused. Capture queue during
   ...
   WF-03.
3. **Relevance floor as part of 028** — refused. A table cannot
