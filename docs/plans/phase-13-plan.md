...
**Date:** 17 Sep 2026
**Status:** **LIVE, not planned.** 13.1 published
...
WF-06 **writes** `enrichment_records`. WF-10 **reads**
them (13.1 LIVE): owner-scoped curated laterals on
`History load` / `Load voice person` / `Load picked
person`, hollow skip, latest `fetched_at`, prompts
`wf10-v6`, Telegram Apollo evidence lines. 88 rows
(46 person/apollo, 36 company/apollo, 6
company/tavily). D-P D-Q D-R D-S D-T are locked. D-U
(echo flag) is recorded, not built.
...
**D-U.** Echo flag, never silent edit. If a draft
repeats an Apollo title/headline that differs from
the card, the confirm card flags it. **Not built.**
Withdrawn packet 13.1 numbered this D-S; D-S is
curated columns.
...
**Rule 27 (G1).** A `person_id` used after an upsert
comes from that upsert's own result, never from a
name lookup. Live WF-05 still name-joins: a no-email
person binds to **every** same-name person (SQL sim:
**3** rows). Do not repair #46 / #208.

**Rule 28 (G2).** The watchdog never requeues an
enrichment job. A stuck spend is parked
`needs_review`; a provider call is never repeated by
recovery. Packet 13.3 PUT on WF-09.

**Rule 29.** A provider balance that cannot be read
blocks the spend; an unknown delta is recorded as 1,
never NaN. Packet 13.3 PUT on WF-06.

### Gap register (architect session 13; G6–G11 quoted)

Owner per row. Do not repair #46 / #208. Packet 13.3
puts G2 (WF-06 + WF-09) and G4 (WF-06) and G5 (WF-09
only). G1 G3 G7 G8 G9 G10 G11 stay open.

| Id | Gap | Owner |
|---|---|---|
| **G1** | After upsert, `person_id` from that upsert’s own result, never a name lookup. Live WF-05 (`b6cd3894`): no-email person binds to **every** same-name person (SQL sim: 3 rows). | Cursor (later PUT). Owner: data |
| **G2** | Watchdog never requeues enrichment. Stuck spend parked `needs_review`. Recovery never repeats a provider call. WF-06: unreadable credits must not call Apollo; `$10` never NaN. | Cursor (13.3) |
| **G3** | WF-04 Insert contact name suggestions `executeOnce`; SQL iterates Parse recordset with owner predicate per row | Cursor |
| **G4** | WF-06 “No email terminal” parks `needs_review` `error_code=no_email`, not `stopAndError` | Cursor (13.3) |
| **G5** | WF-09 List owners / Scan findings resolve event via `bot_state.current_event_id`. Owner without `bot_state` is not listed | Cursor (13.3 WF-09). WF-07 out of this packet |
...
| **G10** | WF-06 settings.binaryMode present | Architect sourced. Strip on PUT; do not send the key |
...
owner + 3 test; 5 note replays 17 Sep 10:00Z). Packet
text that said 15 is stale. Do not backfill.
...
|---|---|---|---|
| 05 | `<WF05_ID>` | `b6cd3894` | 31 |
| 10 | `<WF10_ID>` | `5f6ffbc9` | 167 |
