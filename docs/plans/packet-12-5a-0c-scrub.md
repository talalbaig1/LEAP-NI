...
| Contact names | `<CONTACT_N_NAME>` — same N as that human’s email. Same-human email pairs (1+2, 3+4, 7+8) share the **lower** N. Extra named people continue from 12. Rewrite map uses `regex:(?i)\b…\b`, not literals (so `Amer` cannot eat `American`). Arabic/CJK stay literal. |
| Company-as-person only | `haramaincompanion`, `kacaib`, `kaacib` (and their `.com`). Ordinary company names stay. |
