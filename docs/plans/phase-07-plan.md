...
| `person_id` | uuid | YES | **Required for a sendable draft.** CHECK or gate, not a silent NULL send. |
...
| **7.4 WF-01 wire** | **Only if 29 Aug gate passed.** Append Route `followup`. Callback `f7:`. Optional voice await. `reply_markup` / `reply_text_2` on the command send. PUT without `active`. | Re-GET Route type: `[10]` still Flag arg, `[11]` still Unknown, **new** index is followup, fallback shifted and **re-wired**. `/ask` and `/digest` executions still return and send (regression). `/flag` usage still 265540-class. `POST /activate` is not used on WF-01 (already active). |
