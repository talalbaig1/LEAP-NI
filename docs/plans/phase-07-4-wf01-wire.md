...
| (b) Telegram node, `inlineKeyboard.rows` as **fixed collection entries** (literals) | **WIN.** LNI-TEST-WF10-buttons `<TEST_WF10_BUTTONS_WF_ID>` exec **271606**, `message_id` **359**. API result `reply_markup.inline_keyboard = [[{text:"TAP THIS - LNI prove", callback_data:"f7:t:prove74b"}]]`. |
...
3. PUT did not include `active`. Settings still
   ...
   `errorWorkflow: <WF00_ID>`, `executionTimeout: 300`.
...
2. PUT WF-10: empty-brief compose → true usage line; skip
   ...
   Strip `binaryMode` and `timeSavedMode`.
3. `POST /activate` WF-10 (`<WF10_ID>`) once.
