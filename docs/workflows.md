...
| Owner resolution (Phase 12) | Inbound: `bot_state.telegram_user_id`. Cron: every **tenant** `bot_state.owner_id` (never the platform owner). Fingerprint: `lni_instance` (12.1), **not** `events.name = 'LEAP 2026'` | Q4 locked. After 12.1 the string `LEAP 2026` is not in workflow logic. Do not hardcode `a79b744e`. Do not take owner from `$env`. |
...
**279752**. Packet 9.10 (one PUT, rollback `bc87f636`): both
...
terminals. WF-09 remains the backstop. WF-01 still `4836ffd8`.
...
| 7 WF-01 | GET `4836ffd8`. Unchanged. |
...
WF-01. Live `847cc3c7`, 91 nodes (9.10). Rollback `bc87f636`. Actions:
...
8. Enqueue **ONE** capture-level `job_type='entity_resolution'` row
   ...
   **10.2c-fix PUT 7 Sep.** Rollback `dafe9b02`. Published
   ...
   Abdullah Ahsan; person created. Remaining six: WF-04
...
- Auto-merge people on name. Do not merge Zahir (D-J).
