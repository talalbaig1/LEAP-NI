...
WF-01 PUT is paused (Phase 9). Do not change `<WF01_ROLLBACK>` until the
owner phone test passes. `5df341f8` is a locked follow_up — never
touch. LNI-TEST-7.16-driver `<TEST_716_DRIVER_WF_ID>` inactive unless a
packet activates it.
...
5. **Branch:** command | photo | voice/audio | document/video | text | callback | **contact** | **vcard** | **flag**.
   ...
   **Telegram `contact` — live WF-01 still declines.**
   GET `<WF01_ROLLBACK>`: `Classify update` sets `branch = 'contact'`,
   ...
   **Telegram `.vcf` / vCard — live WF-01 still declines.**
   GET `<WF01_ROLLBACK>`: detect before `branch = 'document'`. Match
...
1. **Enqueue orphan jobs** — the **same** `INSERT … SELECT … ON
   ...
   WF-09 did not until `<WF09_ROLLBACK>`. Do not rewrite them.
...
**ACTIVE** `<WF10_ROLLBACK_9_14>`, 146 nodes. No Telegram trigger. No Schedule.
