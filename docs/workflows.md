...
on the instance — which is ElderWise Postgres (`<ELDERWISE_PG_CRED_ID>`) and
...
| WF-10 | Follow-up drafting | 7 · **10.4 (not built):** `source=history` | Execute Workflow. **ACTIVE.** Called by WF-01 `/followup` / `/done`, WF-02 sweep, WF-05 deferred. Phase 10 adds `source=history` (Gmail Draft + Telegram copy-text, no send). |
...
| Real send | WF-01 **279665** / WF-10 **279667**. `bb3689d8` `draft_state=sent` `gmail_message_id=1a04c9a684523738` `sent_at=08:19:42Z`. To `<CONTACT_5_EMAIL>`. |
...
6. **COMMANDS.** Strip a trailing `@botname`. First token, lowercased,
   ...
   **Packet 4.9 publish (28 Aug 2026).** WF-01 PUT without `active`,
   ...
   `<WF00_ID>`. WF-06 cache SQL already honours `force`;
   no WF-06 edit.
   ...
   **Packet 4.12 measured Part E (28 Aug 2026, 05:55Z).** Messages
   ...
   4. **265857** success last **Flag sent terminal**. Reply
      `Queued <CONTACT_21_NAME> for enrichment. Result within 15 minutes.`
...
that calls an unpublished sub-workflow (`Call WF-02 *` → `<WF02_ID>`).
...
8. Enqueue **ONE** capture-level `job_type='entity_resolution'` row
   ...
   **10.2c-fix PUT 7 Sep.** Rollback `dafe9b02`. Published
   ...
   <CONTACT_36_NAME>; person created. Remaining six: WF-04
   `383365`; WF-05 `383367` (#203) + `383381` (other five).

   **S9 (logged, do not fix).** Replay mint of a person on a
   capture that already has an `interactions` row writes no
   second interaction. `Insert interaction` is `WHERE NOT
   EXISTS (capture_id)`. Proven #151 (null `person_id`) and
   #156–#185 (old person, no recovered contact). #203 had
   no prior row. Outreach still sees the new person. `/ask`
   and digests do not (or see the old row).

   **Call WF-05 is not gated on the insert.** Live `<WF04_PUBLISHED>` had
...
5. **Upsert** `people`, `companies`, `person_companies`, `interactions`.
   ...
   **Person field precedence (packet 3.7) on an email or LinkedIn
   ...
   still `full_name` `'<CONTACT_9_NAME>'` with title `'نائب المدير'` while
   `'<CONTACT_9_NAME>'` / `'Deputy Director'` were available and
...
7. **Capture status (packet 3.7):** set `needs_review` when
   ...
   **#63** are evidence and are **not retro-fixed**.
   Packet 3.9 C merge of <CONTACT_7_NAME> / Huawei is data, not a WF-05 auto-merge.
...
**Live:** **ACTIVE** `<WF06_PUBLISHED>`. Timezone is
...
people (<CONTACT_19_NAME>, <CONTACT_7_NAME>) were also hollow `name` / 0-credit `no_match`
...
1. Tavily hit, <CONTACT_19_NAME> / BTGroup `hasoub.com`. Exec **263839**, last
...
2. Tavily ceiling. `tavily_lifetime_ceiling=0`. <CONTACT_7_NAME> / Qatar
...
**ACTIVE** `<WF10_PUBLISHED_9_14>`, 146 nodes. No Telegram trigger. No Schedule.
...
those with a URL and no email. 8 of 20 emails have a usable
transcript; 12 get the general letter (D-G). Transcripts
are unreliable (D-F).

**Reads**

- Eligible set from Postgres, not a typed name, for the
  recommended batch (Q2). Same lookup ladder if the owner
  names one person. Never silent-pick on `hit_count>1`.
- Stored brief: `interactions.summary` / topics /
...
- Call WhatsApp API or LinkedIn API (D-D / D-E).
- Touch `awaiting_confirm` or the voice picker.
- `UPDATE captures`. Call WF-03. Enqueue enrichment.
- Invent facts, emails, phones, or an ask (D-H).
...
**Batch trap.** A 20-item run must not `executeOnce` the
Gmail draft node (one draft for twenty people). Same class
as WF-04 Call WF-05.
